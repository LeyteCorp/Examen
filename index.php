...
<head>
	<title>Google imagenes</title> <!--Titulo de la pestaña-->
	
	<?php require_once "dependencias.php"; ?> <!--Instanciamos a dependencias.php-->
	<?php 
	require_once "contenido.php"; //Instanciamos a contenido.php
	$datos=contenido();
	?>
</head>
<body>	
	<div class="container">
		<h1>Presentacion de imagenes tipo Google</h1> <!--Titulo uno-->
		<h2>Bestiario</h2> <!--Titulo dos-->
		<!--
		<ul class="gridder">
	...
	<ul class="gridder">
		...
		for ($i=0; $i < count($datos) ; $i++):
			...
			data-griddercontent="<?php echo '#gridder-content-'.$i ?>">
				<img src="<?php echo $d[0] ?>"> <!--Mostramos todas las imagenes-->
			</li>
	...
	<?php
		for ($i=0; $i < count($datos); $i++):
		  	$d=explode("||", $datos[$i]);  
	?>
		<div id="<?php echo 'gridder-content-'.$i; ?>" class="gridder-content" > <!--Esto va hacer que nos jale todos los id que declaramos arriba-->
			<div class="row">
				<div class="col-sm-6">  <!--calss Imagen-->
					<img src="<?php echo $d[0] ?>" class="img-responsive" />
				</div>
				<div class="col-sm-6"> <!--calss Descripcion-->
					<h2><?php echo $d[1]; ?></h2> <!--Titulo-->
					<p><?php echo $d[2]; ?></p>
				</div>
			</div>
		</div>
